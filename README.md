...
| 鑑定結果の自動保存 | AI鑑定レポート生成後に鑑定履歴へ自動保存（二重保存防止付き） |
| セッション永続化 | URLトークンによるセッション切れ時の自動復元でUXを改善 |
| ダークテーマ | ミッドナイトブルー＆ゴールドアクセントのダークテーマ（公式設定＋カスタムCSS） |
...
- **ネットワーク制限**: server.addressを`127.0.0.1`に制限（外部ネットワークからのアクセス防止）
