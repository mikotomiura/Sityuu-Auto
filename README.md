...
出品者（メンター）が相談者の生年月日を入力し、**四柱推命・算命学の命式を瞬時に算出**、AIが鑑定レポートや傾聴のヒントを生成するWebアプリケーション（SaaS）です。マルチユーザー認証・アカウント間データ分離・個別APIキー管理（BYOK）・メールによるセルフパスワードリセットに対応しています。
...
| パスワードリセット | 管理者発行リセットリンク + メールによるセルフサービスリセット（SMTP設定時） |
| メールアドレス登録 | 設定画面・新規登録時にメールアドレスを登録。セルフパスワードリセットに使用 |
| 管理者ページ | ユーザー一覧・削除、招待リンク生成・管理、パスワードリセット（adminロール限定） |
| アカウント間データ分離 | 相談者・鑑定履歴はアカウントごとに完全分離。他ユーザーのデータは閲覧不可 |
...
#### 方法1: セルフサービスリセット（SMTP設定時・推奨）

メールアドレスを登録済みのユーザーは、管理者に連絡せず自力でリセットできます。

1. ログイン画面の **「パスワードを忘れた方」** をクリック
2. 登録済みのメールアドレスを入力
3. メールで届くリセットリンクをクリック
4. 新しいパスワードを設定 → ログイン

> この機能を有効にするには、`.env` に SMTP設定（`SMTP_HOST`, `SMTP_FROM` 等）が必要です。

#### 方法2: 管理者によるリセット

メールアドレス未登録の場合やSMTP未設定の場合は、管理者がリセットリンクを発行します。

1. 管理者アカウントでログイン
2. **「管理」→「パスワードリセット」** タブで対象ユーザーを選択
3. **「リセットリンクを生成」** → URLを対象ユーザーに共有
4. ユーザーがURLにアクセスし新しいパスワードを設定

> リセットリンクはワンタイム（1回使用で無効化）で、有効期限（デフォルト24時間）を設定できます。
> リセット完了時に既存のログインセッションは自動的に無効化されます。

### メールアドレスの登録

パスワードのセルフリセットに必要です。以下のいずれかの方法で登録できます。

- **新規登録時**: 招待リンクからのアカウント作成時に任意入力
- **ログイン後**: **設定 → アカウント設定** でメールアドレスを登録・変更
...
| パスワードリセット | ユーザーを選択してリセットリンクを生成・一覧・削除 |
...
│   └── 05_admin.py           # 管理者ページ（ユーザー管理・招待リンク・パスワードリセット）
...
│   │   └── password_reset_repo.py # パスワードリセットトークンリポジトリ
│   └── migrations/           # SQLマイグレーション
└── utils/                    # 共通ユーティリティ
    ├── auth.py               # 認証（ログイン・セッション管理・セルフリセット）
    ├── email_sender.py       # メール送信（SMTP経由パスワードリセット）
...
- **パスワードリセット**: ワンタイムトークン（256ビット）。リセット完了時に既存セッションを自動無効化
- **管理者ページ保護**: ナビゲーション制御＋ページ内ロールチェック＋`st.stop()` による二重防御
- **データ分離**: 相談者・鑑定セッションは `user_id` カラムでアカウントごとに完全分離。全クエリに所有者フィルタを強制
