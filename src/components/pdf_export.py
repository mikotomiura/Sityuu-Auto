...
def _ensure_fonts_registered() -> bool:
    ...
    初回呼び出し時のみフォント登録を行い、以降はキャッシュで即座に返る。

    Returns:
        常に True（lru_cache のキーとして使用）。
    """
...
def _markdown_to_paragraphs(
    ...
    for line in text.split("\n"):
        ...
        if stripped.startswith("### "):
            ...
            elements.append(Paragraph(f"・ {_escape_xml(bullet_text)}", styles["body"]))
        elif stripped.startswith("| "):
            # テーブル行は本文として出力
...
def build_reading_report_pdf(
    ...
    *,
    ...
    try:
        ...
        # --- タイトル ---
        elements.append(Paragraph(f"鑑定レポート: {_escape_xml(name_display)}", styles["title"]))
        ...
        elements.append(Paragraph(f"Generated by Sityuu-Auto on {today}", styles["caption"]))
...
def build_fallback_pdf(
    ...
    try:
        ...
        elements.append(Paragraph(f"鑑定レポート: {_escape_xml(client_name)}", styles["title"]))
        elements.append(Paragraph(f"鑑定日: {today}", styles["caption"]))
        elements.append(Spacer(1, 6 * mm))
